...
describe("scaffoldPlugin SDK dependency", () => {
  ...
  it("uses the canonical id in a scoped package scaffold", async () => {
    ...
    // The generated skill owns operational judgment, not just command
    // discovery: fresh agents must inspect current state, preserve completed
    // work, and recover from stale ids without editing storage directly.
