...
selected environment or machine catalog before create and selection-bearing
updates persist them. Prompt-only edits and target-thread automations do not
probe an unused model catalog. Spawn validates again at run time before
thread/environment provisioning. An invalid model or an advertised unsupported
reasoning level exits non-zero.
