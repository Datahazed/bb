...
describe("@bb/sdk", () => {
  ...
  it("manages push subscriptions through the notifications area", async () => {
    const subscription = {
      id: "push_123",
    ...
    await expect(
      sdk.notifications.pushSubscriptions.add(request),
    ).resolves.toEqual({ created: true, subscription });
    await expect(
      sdk.notifications.pushSubscriptions.add(request),
    ).resolves.toEqual({
      created: false,
    ...
    expect(queue.requests).toEqual([
      {
        ...
        method: "POST",
        url: "http://bb.test/api/v1/notifications/push-subscriptions",
      },
      {
        bodyText: JSON.stringify(request),
        method: "POST",
      ...
      },
      {
        bodyText: undefined,
        method: "DELETE",
        url: "http://bb.test/api/v1/notifications/push-subscriptions/push_123",
      },
      {
        bodyText: undefined,
        method: "DELETE",
