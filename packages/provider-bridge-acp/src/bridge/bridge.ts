...
function handleAgentNotification(
  ...
  if (session.providerThreadId === "") {
    ...
    // The agent-turn bracket waits too: a turn/started emitted before
    // thread/identity would name a session bb has not been told about, and
    // the session.reset that follows identity would drop it anyway.
