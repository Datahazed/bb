 ...
 * document root and `@property` is invalid when nested. The `:root` block
 * carries only the non-inline tokens (Tailwind defaults such as
 * `--container-3xl`, the host's typography scale) that the plugin's utilities
 * reference by `var()` and the host emits only for the ones it uses itself;
 * the semantic bridge is `inline reference` (plugin-theme.generated.ts) and
 * emits nothing.
 */
