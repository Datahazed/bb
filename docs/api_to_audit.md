...
single-section view state and a setter, so a plugin can implement a direct
focus toggle without owning sidebar markup.
...
when several plugins contribute pressed actions; decide whether single-section
view state should include plugin ownership or remain a section id; test disable,
