...
import { extractEnvOverrides } from "@bb/provider-driver-helpers/provider-utils";
import { withoutBridgeRuntimeEnv } from "@bb/provider-driver-helpers/bridge-runtime-env";
import { shouldAutoDenyInteractiveRequest } from "./permission-policy.js";
...
import {
  claudeCodeLaunchConfigSchema,
  type ClaudeCodeLaunchConfig,
...
let launchConfig: ClaudeCodeLaunchConfig = claudeCodeLaunchConfigSchema.parse(
  {},
);
...
function claudeSkillPluginName(
  source: ProviderSessionOpenParams["skillSources"][number],
): string {
  const identityHash = createHash("sha256")
    .update(source.id)
    .update("\0")
    .update(source.rootPath)
    .digest("hex")
    .slice(0, 16);
  return `bb-skills-${identityHash}`;
}

async function writeClaudeSkillPluginManifest(
  source: ProviderSessionOpenParams["skillSources"][number],
): Promise<void> {
  const manifestDirectoryPath = join(source.rootPath, ".claude-plugin");
  const manifestPath = join(manifestDirectoryPath, "plugin.json");
  ...
  try {
    ...
    // The generic skill package has no provider-native manifest yet.
  }

  const temporaryManifestPath = join(
    manifestDirectoryPath,
    `.plugin-${process.pid}-${randomUUID()}.json`,
  );
  try {
    await writeFile(
      ...
      `${JSON.stringify(
        {
          ...
          name: claudeSkillPluginName(source),
          version: "0.1.0",
          description: "Skills staged by bb.",
          author: { name: "bb" },
          skills: source.skills.map((skill) => `./skills/${skill.name}`),
        },
        null,
        2,
      )}\n`,
...
async function prepareClaudeSkillPlugins(
  skillSources: ProviderSessionOpenParams["skillSources"],
): Promise<void> {
  await Promise.all(skillSources.map(writeClaudeSkillPluginManifest));
}
...
function resolveClaudePermissionMode(
  ...
  if (params.execution.features.planModeEnabled) return "plan";
...
async function createCanonicalSession(
  ...
  await prepareClaudeSkillPlugins(params.skillSources);
  const threadIdRef = { current: params.bbThreadId };
  const preparedEnv = await prepareSessionEnv({
    claudeCodeMockCliTraffic: launchConfig.mockCliTraffic,
...
export const claudeCodeProviderDriver = defineProviderDriver({
  ...
  initialize(params) {
    launchConfig = claudeCodeLaunchConfigSchema.parse(params.config);
  },
