...
describe("Claude Code canonical provider driver", () => {
  ...
  it("prepares Claude plugin metadata for generic staged skill packages", async () => {
    const skillRootPath = await mkdtemp(
      join(tmpdir(), "bb-claude-skill-package-"),
    );

    try {
      const { value } = context();
      await driver.openSession(
        providerSessionOpenParamsSchema.parse({
          ...sessionParams(),
          skillSources: [
            {
              id: "global-skills:test-catalog",
      ...
      await expect(
        readFile(
          join(skillRootPath, ".claude-plugin", "plugin.json"),
          "utf8",
        ).then((content) => JSON.parse(content)),
      ).resolves.toMatchObject({
        name: expect.stringMatching(/^bb-skills-[a-f0-9]{16}$/u),
        skills: ["./skills/release-notes"],
      });
    } finally {
      await rm(skillRootPath, { recursive: true, force: true });
    }
  });
