...
import { decodeNativeProviderToolCallRequest } from "@bb/provider-driver-helpers/provider-tool-call-contract";
...
import { buildShellEnvironmentPolicyConfig } from "@bb/provider-driver-helpers/provider-utils";
import type { ProviderExecutionContext } from "./driver-types.js";
...
function toProviderExecutionContext(
  ...
  return {
    ...
    providerOptions: params.execution.providerOptions,
...
async function openCodexSession(
  ...
  await connection.request({
    ...
    params: {
      extraRoots: params.skillSources.map((source) =>
        path.join(source.rootPath, "skills"),
      ),
    },
    resultSchema: z.unknown(),
  });
