...
import { z } from "zod";
import { buildEditDiff } from "@bb/provider-driver-helpers/provider-utils";
import { withoutBridgeRuntimeEnv } from "@bb/provider-driver-helpers/bridge-runtime-env";
import { mimeTypeFromExtension } from "@bb/provider-driver-helpers/mime-types";
  ...
  const lines = skillSources.flatMap((source) =>
    source.skills.map((skill) => {
      const description = (skill.description ?? "(description unavailable)")
        ...
        .replace(/[<>]/gu, "")
        .trim();
      return `- ${skill.name}: ${description || "(description unavailable)"} (SKILL.md: ${resolve(source.rootPath, "skills", skill.name, "SKILL.md")})`;
    }),
  );
...
function buildModelSelection(args: {
  ...
  model: string;
  modelCli?: NonNullable<AcpLaunchConfig["modelCli"]>;
  nativeReasoning?: AcpLaunchConfig["nativeReasoning"];
  reasoningLevel: ReasoningLevel;
...
function profileFromInitialization(
  ...
  const parsed = acpLaunchConfigSchema.parse(context.initialization.config);
  return normalizeAcpLaunchConfig(parsed);
}

async function listModelsForProfile(profile: AcpLaunchConfig) {
  const listCommand =
