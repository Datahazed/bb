...
describe("PaneMaximizeButton", () => {
  ...
  it("uses the shared thread-header icon geometry", () => {
    renderButton(false);
    const button = screen.getByRole("button", { name: "Full Screen (⌘⇧E)" });
    for (const token of THREAD_HEADER_ICON_BUTTON_CLASS.split(/\s+/)) {
      expect(button.classList.contains(token), `missing ${token}`).toBe(true);
    }
    expect(button.classList.contains("[&_svg]:size-[16px]")).toBe(true);
    expect(button.classList.contains("[&_svg]:size-[13px]")).toBe(false);
  });
