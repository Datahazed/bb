...
function normalizeHighlightRanges(
  ...
  for (const range of ranges) {
    ...
    // Malformed input can point into an astral character. Expand the highlight
    // to the complete code point so consumers never receive a slicing boundary
    // between its high and low surrogates.
...
/**
 * Builds the palette's compact display window around the first valid match.
 * The caller decides whether its one-line clamp would hide that match; calling
 * this helper explicitly requests the 16-character lead and 40-character tail.
 */
export function windowPaletteThreadSearchText({
  ...
  // Prefer dropping the astral character straddling a hard window boundary to
  // returning either half of it. Word-boundary moves already land safely.
  if (isInsideSurrogatePair(text, start)) {
    start += 1;
  }
  if (isInsideSurrogatePair(text, end)) {
