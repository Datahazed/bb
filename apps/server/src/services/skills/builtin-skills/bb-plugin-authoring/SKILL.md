...
- `bb.experimental_hostDrivers` (optional) — declares isolated Node host-driver
  source entries as `{ id, entry }`. IDs are lowercase kebab-case and unique
  within the plugin; entries must be files confined beneath the plugin root.
  `bb plugin build` bundles each entry for Node 22 and writes
  `dist/host/<id>/driver.{js,js.map,meta.json,tgz}`. Git/path installs build
  declarations; npm installs must publish matching metadata and archives.
  This artifact declaration is experimental and does not itself register a
  selectable provider.
