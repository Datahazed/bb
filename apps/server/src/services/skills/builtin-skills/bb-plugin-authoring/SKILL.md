...
Plugins created before the public package may keep their legacy
`@bb/plugin-sdk` and `@bb/plugin-sdk/app` source imports: the builder and host
resolve both names to the same runtime. New source and all npm testing imports
use `@get-bb/plugin-sdk`; the legacy scope is not published.
