/**
 * Ported from `apps/host-daemon/src/codex-auth.ts` (P1a engine scaffold; the
 * daemon copy dies in P1c).
 *
 * Reads `~/.codex/auth.json` directly — the server inherits the daemon's
 * interactive-user assumption (plan R8, documented constraint): it must run
 * as the user who ran `codex login`. Read-only: tokens are never refreshed
 * or written back.
 */
