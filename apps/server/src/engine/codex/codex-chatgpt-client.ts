/**
 * Ported from `apps/host-daemon/src/codex-chatgpt-client.ts` (P1a engine
 * scaffold; the daemon copy dies in P1c). No daemon-transport dependencies:
 * this module talks to chatgpt.com / api.openai.com directly (codex inference
 * proxy + voice transcription) with SSE and size-bounded body readers, using
 * the process-global Cloudflare service-cookie jar.
 *
 * `../core/command-dispatch.ts`'s durable handler map imports
 * `completeCodexInference` / `transcribeCodexVoice` for
 * `codex.inference.complete` / `codex.voice.transcribe` — keep the export
 * names stable.
 */
