...
import type { ReplayTaskRegistry } from "../core/command-dispatch-support.js";
import type { EngineThreadEventInput, ThreadEventSink } from "../ports.js";
...
function captureEventSink(emitted: EngineThreadEventInput[]): ThreadEventSink {
  return {
    emit: (event) => {
      emitted.push(event);
    },
...
describe("replay capture commands", () => {
  it("lists readable capture manifests from the engine data dir", async () => {
  ...
  it("loads capture detail from the engine data dir", async () => {
...
describe("runReplay", () => {
  it("replays raw provider records through translation with remapped thread ids", async () => {
    ...
    const emitted: EngineThreadEventInput[] = [];
  ...
  it("uses the replayed turn id when aborting after turn start", async () => {
    ...
    const eventSink: ThreadEventSink = {
      emit: (event) => {
        emitted.push(event);
