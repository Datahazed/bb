...
} from "../environment/injected-skills.js";
 ...
 * refresh instead. Reaching this error indicates an engine bug.
 */
export class SkillCatalogConflictError extends Error {
  constructor(args: SkillCatalogConflictErrorArgs) {
    super(
      `Engine bug: a command targeting no thread carried injected skill sources into busy environment ${args.environmentId} (active catalog ${args.activeCatalogHash ?? "none"}, requested ${args.requestedCatalogHash})`,
    );
...
export class RuntimeManager {
  ...
  async shutdownAll(): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      // lifecycle via explicit environment.destroy commands. Engine shutdown
