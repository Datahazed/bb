/**
 * Adapted from `apps/host-daemon/src/app-data-change-reporter.ts` (P1a engine
 * scaffold; the daemon copy dies in P1c). The HTTP half was the transport
 * seam: `postAppDataChange`/`postAppDataResync` are re-typed to the
 * `AppDataChangeSink` port members (P1b passes `ports.appData.publishChange`/
 * `.publishResync`, which call `hub.notifyAppData` directly). The
 * version-cache, generation counter, and per-key observe serialization are
 * preserved verbatim — they are the in-memory dedupe state that keeps
 * `bb.data.onChange` quiet on no-op writes — as is the warn-only failure
 * handling.
 */
import {
  appDataPathSchema,
...
interface CreateAppDataChangeReporterOptions {
  logger: EngineLogger;
  postAppDataChange: AppDataChangeSink["publishChange"];
  postAppDataResync: AppDataChangeSink["publishResync"];
}
...
export class AppDataChangeReporter {
  ...
  requestResync(args: HostDaemonAppDataResyncPayload): Promise<void> {
  ...
  private async postResyncHint(
    args: HostDaemonAppDataResyncPayload,
  ): Promise<void> {
    try {
