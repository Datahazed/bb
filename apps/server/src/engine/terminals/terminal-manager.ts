/**
 * Adapted copy of `apps/host-daemon/src/terminals/terminal-manager.ts`
 * (strangler P1a — not yet reachable from any server runtime path).
 * In-memory semantics are unchanged: node-pty adapter (incl. the darwin
 * spawn-helper chmod machinery), per-session scrollback ring (4 MiB /
 * 10k seq-numbered chunks; scrollback dies with the process), session map,
 * and terminal env construction (`BB_TERMINAL_SESSION_ID` + sanitized
 * inherited env + `runtimeManager.getShellEnv()`).
 *
 * Transport adaptations, all via `../ports.js`:
 * - `handleMessage` takes `EngineTerminalCommand`, replacing the daemon's
 *   `HostDaemonServerTerminalMessage` (the server→daemon WS half).
 * - Emissions go through `SendTerminalEvent`, which returns void — the
 *   daemon's boolean WS-send result was never consumed.
 * - `shutdownAll` requires an explicit close reason; the daemon's
 *   `"daemon-disconnect"` default was a transport-dead value.
 */
...
import { ExpectedCommandDispatchError } from "../core/command-dispatch-support.js";
import { runtimeErrorLogFields } from "../core/error-utils.js";
import type { RuntimeManager } from "../core/runtime-manager.js";
...
const SCROLLBACK_MAX_BYTES = 4 * 1024 * 1024;
const SCROLLBACK_MAX_CHUNKS = 10_000;
...
export interface TerminalManagerOptions {
  ...
  sendMessage: SendTerminalEvent;
}

interface ScrollbackEntry {
  byteLength: number;
  chunk: Extract<EngineTerminalEvent, { type: "terminal.output" }>["chunk"];
}
...
export function ensureNodePtySpawnHelperExecutable(logger: EngineLogger): void {
...
export class TerminalManager {
  ...
  async handleMessage(message: EngineTerminalCommand): Promise<void> {
    switch (message.type) {
      case "terminal.open":
  ...
  async shutdownAll(reason: TerminalSessionCloseReason): Promise<void> {
  ...
  private pruneScrollback(session: TerminalSession): void {
    while (
      session.scrollbackBytes > SCROLLBACK_MAX_BYTES ||
      session.scrollback.length > SCROLLBACK_MAX_CHUNKS
    ) {
