...
import type { EngineLogger, EngineTerminalEvent } from "../ports.js";
import {
...
function createFakeLogger(): EngineLogger {
  return {
    debug: vi.fn(),
    error: vi.fn(),
    info: vi.fn(),
...
function createHarness(): TerminalManagerHarness {
  ...
  const messages: EngineTerminalEvent[] = [];
...
function collectTerminalOutput(messages: EngineTerminalEvent[]): string {
  return messages
    .flatMap((message) =>
...
describe("TerminalManager", () => {
  ...
  it("kills all terminals on shutdown with the caller-supplied reason", async () => {
    ...
    await harness.manager.shutdownAll("daemon-disconnect");
    pty.emitExit(0);
  ...
  it(
    ...
    async () => {
      ...
      await manager.shutdownAll("daemon-disconnect");
    },
    10_000,
  );
});
