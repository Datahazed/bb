...
describe("public thread data routes", () => {
  ...
  it.each([
    {
      activeList: "workflow" as const,
      taskId: "task:wf-window",
      taskType: "local_workflow" as const,
      workflowName: "fixture-window",
    },
    {
      activeList: "backgroundCommand" as const,
      taskId: "task:bash-window",
      taskType: "local_bash" as const,
      workflowName: null,
    },
  ])(
    "surfaces active $taskType background tasks outside the latest timeline window",
    async ({ activeList, taskId, taskType, workflowName }) => {
      await withTestHarness(async (harness) => {
        const { environment, thread } = seedThreadFixture(harness);
        ...
        const workflowTaskData = {
          item: {
            ...
            id: taskId,
            taskType,
            description: "windowed background task",
