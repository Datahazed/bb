...
import {
  deletableSkillScopeSchema,
  editableSkillScopeSchema,
  skillScopeSchema,
...
export function registerSkillCommands(
      ...
      action(async (options: SkillWorkspaceOptions) => {
        ...
        console.log(
          renderBorderlessTable(
            {
              head: ["NAME", "SCOPE", "PROVIDER", "EDITABLE", "PATH"],
              colWidths: [24, 18, 14, 10, 48],
  ...
  addWorkspaceOptions(skill.command("show <scope> <name>"))
      ...
      action(
        async (
          ...
          const result = await createCliBbSdk(getUrl()).skills.getContent({
            projectId: projectId(options, getContext()),
            environmentId: environmentId(options),
            scope: skillScopeSchema.parse(scopeValue),
            name,
            path: options.path,
          });
          if (outputJson(options, result)) return;
  ...
  addWorkspaceOptions(skill.command("files <scope> <name>"))
  ...
  addWorkspaceOptions(skill.command("update <scope> <name>"))
      ...
      action(
        async (
          ...
          const result = await createCliBbSdk(getUrl()).skills.update({
            projectId: projectId(options, getContext()),
            environmentId: environmentId(options),
            scope: editableSkillScopeSchema.parse(scopeValue),
            name,
  ...
  addWorkspaceOptions(skill.command("delete <scope> <name>"))
        ...
        async (
          ...
          if (
            ...
            !(await confirmDestructiveAction(
              `Delete ${scopeValue}/${name}? This cannot be undone.`,
            ))
          ) {
            console.log("Aborted.");
            return;
          ...
          const result = await createCliBbSdk(getUrl()).skills.remove({
            projectId: projectId(options, getContext()),
            environmentId: environmentId(options),
            scope: deletableSkillScopeSchema.parse(scopeValue),
            name,
          });
          if (outputJson(options, result)) return;
