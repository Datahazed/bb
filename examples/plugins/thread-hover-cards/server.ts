...
export const threadSummarySchema = z
  .object({
    ...
    permissionMode: z
      .enum(["accept-edits", "auto", "full", "readonly", "workspace-write"])
      .nullable(),
...
function latestTimelineAssistantMessage(
...
function recordSkippedStage(recorder: DiagnosticsRecorder, name: string): void {
  recorder.stages.push({
    cache: "none",
    durationMs: 0,
    name,
...
async function withinCached<T>(
  ...
  return (await within(promise, timeoutMs)) ?? { source: "miss", value: null };
}
...
/**
 * Matches the rows a section actually renders: visible, unarchived roots that
 * are not pinned, because bb lifts pinned threads out of their section into the
 * Pinned group (apps/app/src/components/sidebar/ProjectList.tsx). The SDK's
 * ordinary thread list does not return side chats.
 */
...
export default function plugin(bb: BbPluginApi): void {
  const recordBackgroundRefresh: BackgroundCacheRefreshObserver = (timing) => {
    bb.log.debug(`thread-hover-cards:cache-refresh ${JSON.stringify(timing)}`);
  };
          ...
          isRunningStatus(thread.runtime.displayStatus)
            ...
            : measureStage(
                ...
                const latestMessage = timeline
                  ...
                  : null;
                return (
                  latestMessage ?? loadOutlineMessage("messageOutlineFallback")
                );
              });
    ...
    async threadPullRequest({ threadId }) {
      ...
      try {
        ...
        const pullRequestResult = await measureCachedStage(
          ...
          "pullRequest",
          () =>
            pullRequests.get(pullRequestCacheKey, async () => {
