...
test("serves thread and section hover-card data", async () => {
  type SummaryHandler = (input: {
    clientId?: string;
    generation?: number;
    threadId: string;
  }) => Promise<ThreadSummary>;
  type TimingHandler = (input: { threadId: string }) => Promise<ThreadTiming>;
    ...
    sdk: {
      ...
      projects: {
        ...
        async get() {
          ...
          return {
            gitRemoteUrl:
              "git@github.com:acme/bb-plugin-thread-hover-cards.git",
            name: "Thread cards",
          };
        },
      },
  ...
  function stage(
    ...
    const result = diagnostics.stages.find(
  ...
  assert.ok(pullRequestHandler, "registers the threadPullRequest RPC handler");
  ...
  const otherBranchPullRequest = await pullRequestHandler({
    threadId: "thr_1",
  });
  ...
  assert.equal(
    aggregates.failed,
    1,
    "failures are counted apart from questions",
  );
