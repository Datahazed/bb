...
test("renders and cleans up thread and section hover cards", async () => {
  const dom = new JSDOM(
    `<head>
  ...
  const abortedTimingThreadIds = [];
  const abortedPullRequestThreadIds = [];
  ...
  globalThis.fetch = async (url, init) => {
    ...
    if (String(url).endsWith("/threadTiming")) {
      ...
      return (
        deferResponse({
          abortedIds: abortedTimingThreadIds,
    ...
    if (String(url).endsWith("/threadPullRequest")) {
      ...
      return (
        deferResponse({
          abortedIds: abortedPullRequestThreadIds,
  ...
  const { loadPluginApp, mountPluginContentScripts } =
    await import("@bb/plugin-sdk/testing/app");
  const pluginApp = await loadPluginApp(() => import("../app.tsx"));
  assert.deepEqual(
    pluginApp.contentScripts.map(({ id }) => id),
    ["thread-hover-cards"],
  );
  const mountedContentScripts = await mountPluginContentScripts(pluginApp, {
    generation: 7,
    pluginId: "thread-hover-cards",
  });
  assert.deepEqual(mountedContentScripts.inspection.mountedIds, [
    "thread-hover-cards",
  ]);
  ...
  assert.doesNotMatch(style.textContent, /color-mix\(in srgb/);
  ...
  assert.equal(
    card.querySelector(
      ".bb-thread-hover-card__runtime .bb-thread-hover-card__sr-only",
    )?.textContent,
    "Run time ",
  );
  assert.equal(
  ...
  assert.equal(
    card.querySelector(
      ".bb-thread-hover-card__provider .bb-thread-hover-card__sr-only",
    )?.textContent,
    "Codex, ",
  );
  assert.equal(
  ...
  assert.equal(
    card.querySelector(".bb-thread-hover-card__inline-emphasis"),
    null,
  );
  ...
  assert.equal(
    card.querySelector(".bb-thread-hover-card__provider-model")
      ?.nextElementSibling,
  ...
  assert.equal(
    card.querySelector(
      ".bb-thread-hover-card__pr .bb-thread-hover-card__meta-label",
    ),
    null,
  );
  ...
  assert.equal(
    card
      .querySelector(".bb-thread-hover-card__local")
      ?.getAttribute("aria-label"),
  ...
  assert.deepEqual(
    Array.from(card.children).map((child) => child.className),
    ["bb-thread-hover-card__header", "bb-thread-hover-card__context"],
  );
  assert.deepEqual(requestBodies, [
    { threadId: "thr_1" },
    { threadId: "thr_1" },
  ...
  assert.equal(
    window.document.getElementById("bb-thread-hover-card-styles"),
    null,
  );
  ...
  assert.equal(
    reloadedCard.querySelector(
      ".bb-thread-hover-card__runtime [data-time-value]",
    )?.textContent,
    "1m",
  );
  assert.equal(
  ...
  assert.equal(
    reloadedCard.querySelector(".bb-thread-hover-card__summary")?.dataset
      .working,
    undefined,
  ...
  await new Promise((resolve) => setTimeout(resolve, 20));
  assert.equal(
    requestBodies.filter(({ threadId }) => threadId === "thr_lru_recent")
      .length,
  ...
  assert.equal(
    sectionCard.querySelector(".bb-section-hover-card__chip--failed")
      .textContent,
    "1 failed",
  );
  assert.equal(
  ...
  assert.equal(
    sectionCard.hidden,
    false,
    "keeps a parent-tier project section card open after card insertion",
  );
  ...
  assert.equal(
    sectionCard.hidden,
    false,
    "keeps a plain project section card open after card insertion",
  );
  ...
  await new Promise((resolve) => setTimeout(resolve, 20));
  assert.equal(
    abortedSectionIds.filter((sectionId) => sectionId === "sec_delayed_a")
      .length,
  ...
  await new Promise((resolve) => setTimeout(resolve, 0));
  assert.equal(
    rebindHeader.toggle.hasAttribute("aria-describedby"),
    false,
    "clears the stale toggle association after replacement",
  );
  assert.equal(
    replacementHeader.toggle.getAttribute("aria-describedby"),
    "bb-section-hover-card",
    "moves the card association to the current toggle",
  );
  assert.equal(sectionCard.hidden, false);
  ...
  assert.equal(
    replacementHeader.toggle.hasAttribute("aria-describedby"),
    false,
    "clears the removed toggle association",
  );
  ...
  delayNextTimingFor.add("thr_dispose");
  delayNextPullRequestFor.add("thr_dispose");
  await closeAndOpenThread("thr_dispose", 20);
  assert.ok(delayedTimingResponses.has("thr_dispose"));
  assert.ok(delayedPullRequestResponses.has("thr_dispose"));

  await mountedContentScripts.lifecycle.dispose();
  assert.equal(mountedContentScripts.inspection.signal.aborted, true);
  assert.equal(mountedContentScripts.inspection.disposed, true);
  assert.ok(abortedTimingThreadIds.includes("thr_dispose"));
  assert.ok(abortedPullRequestThreadIds.includes("thr_dispose"));
  assert.equal(
    window.document.getElementById("bb-thread-hover-card-styles"),
    null,
  );
  assert.equal(
    window.document.getElementById("bb-section-hover-card-styles"),
    null,
  );
