...
const PLUGIN_CSS_SELECTOR = 'link[data-bb-plugin-css="thread-hover-cards"]';
...
interface ClientTimingRecord {
  ...
  durationMs: number;
  operation: "firstContent" | "open" | "pullRequest" | "summary" | "timing";
...
type HugeiconDefinition = readonly (readonly [
  string,
...
function isProjectHeaderRow(row: Element): boolean {
  return (
    row
      .closest(STICKY_GROUP_SELECTOR)
      ?.parentElement?.matches(PROJECT_ITEM_SELECTOR) === true
  );
}
...
function refreshRunTime(card: HTMLElement): void {
  ...
  if (runtime) {
    ...
    runtime.querySelector<HTMLElement>("[data-time-value]")!.textContent =
      value;
...
function appendInlineMarkdown(
  ...
  while (remaining) {
    ...
    if (next.type === "code") {
      parent.append(
        element("code", "bb-thread-hover-card__inline-code", value),
      );
  ...
  if (!summary.repository.isGitRepository) {
    const localContext =
      ...
      (summary.repository.name === "Repository unavailable"
        ? "Local"
        : summary.repository.name);
    const local = element("section", "bb-thread-hover-card__local");
...
function installHoverCards({
  onOpen,
}: ThreadHoverCardOptions): HoverCardController {
  ...
  const pending = new Map<string, PendingRequest<ThreadSummary>>();
  const pullRequestPending = new Map<
    string,
    PendingRequest<ThreadPullRequest>
  >();
  const timingPending = new Map<string, PendingRequest<ThreadTiming>>();
  ...
  function cacheSummary(threadId: string, summary: ThreadSummary): void {
    ...
    const shouldKeepTimingValues =
      hasHydratedTiming &&
      (previous.summary.status === summary.status || timingIsNewerThanSummary);
  ...
  function requestTiming(threadId: string): Promise<ThreadTiming> {
    ...
    const startedAt = monotonicNow();
    const controller = new AbortController();
    let request: Promise<ThreadTiming>;
    request = fetchTiming(threadId, controller.signal)
      .then((timing) => {
        recordTiming({
          cache: "miss",
          durationMs: elapsedMs(startedAt),
      ...
      .finally(() => {
        if (timingPending.get(threadId)?.promise === request) {
          timingPending.delete(threadId);
        }
      });
    timingPending.set(threadId, { controller, promise: request });
    return request;
  }
  ...
  function requestPullRequest(
    ...
    const startedAt = monotonicNow();
    const controller = new AbortController();
    let request: Promise<ThreadPullRequest>;
    request = fetchPullRequest(threadId, controller.signal)
      .then((pullRequest) => {
        recordTiming({
          cache: "miss",
          durationMs: elapsedMs(startedAt),
      ...
      .finally(() => {
        if (pullRequestPending.get(requestKey)?.promise === request) {
          pullRequestPending.delete(requestKey);
        }
      });
    pullRequestPending.set(requestKey, { controller, promise: request });
    return request;
  }
  ...
  function showCard(
    ...
    void requestSummary(threadId, cached ? "stale" : "miss", generation)
      .then((summary) => {
        if (
          disposed ||
          generation !== requestGeneration ||
  ...
  return {
    dispose() {
      ...
      pending.clear();
      for (const request of pullRequestPending.values()) {
        request.controller.abort();
      }
      pullRequestPending.clear();
      for (const request of timingPending.values()) request.controller.abort();
      timingPending.clear();
...
function installSectionHoverCards({
  ...
  const cache = new Map<
    string,
    { fetchedAt: number; summary: SectionSummary }
  >();
  ...
  function showCard(target: SectionTrigger): void {
    ...
    if (
      cached &&
      Date.now() - cached.fetchedAt < SECTION_SUMMARY_CACHE_TTL_MS
    ) {
      return;
    }
